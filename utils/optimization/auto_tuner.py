...
# Import DataType and ClassificationResult
try:
    from core.l0_classifier.classifier import DataType, ClassificationResult
except ImportError:
    try:
        from src.layer0_classifier import DataType, ClassificationResult
    except ImportError:
        # Fallback: define minimal stubs if classifier unavailable
...
class AutoTuner:
    ...
    def recommend(self, classification: ClassificationResult) -> PipelineConfig:
        ...
        self.logger.info(
            f"Recommended config: mode={mode}, type={data_type.value if hasattr(data_type, 'value') else data_type}, "
