"""Core copy of protocol_bridge used by the reorganized layers.

Keeping a mirror of `src/protocol_bridge.py` here allows the
restructured `core` packages to import from a location that is
self-contained and independent of the backward‑compatibility stubs
in `src`.

This file should be kept in sync with the `src` copy; it does not
implement any new behaviour.
"""
